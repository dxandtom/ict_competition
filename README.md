...
| 4 | **最大化发挥 LLM；结合传统 + 新颖方法** | 将传统的 SCA/SAST/DAST 与 **LLM 语义 SAST**（多跳污点分析 + 缺失检查推理）、一个**多审计员交叉验证小组**（内存安全 / 整数溢出 / 反序列化 / 并发 / 红队）以及**新颖的 ML 视角**相融合：对数值内核进行不变量 / 差分 / 蜕变性质测试，从而发现单靠模糊测试会遗漏的契约缺陷。 |
...
```bash
# 1) 把目标代码放入 code/（该技能将其视为未知的黑盒）。
# 2) 在 Claude Code 中调用该技能：
/ai-vuln-hunt
# 3) 产物输出到 findings/：ledger.jsonl、env_manifest.json、sbom/、raw/、candidates/、
#    findings/VH-NNNN/（证明包）、unconfirmed/，以及 REPORT.md。
```
...
.claude/skills/ai-vuln-hunt/
  SKILL.md                  # 端到端操作手册（方法论本体）
  scripts/                  # 账本、黑盒守护、SCA/SAST/DAST 工具、confirm_finding、自检
  templates/                # 审查提示词、sink 分类、测试桩、JSON 模式、报告
  examples/oob_demo/        # 一个已捕获、可过门的完整示例
code/                       # （你添加）目标代码库——按黑盒处理
findings/                   # （生成）可复现的审计轨迹 + 证明包 + REPORT.md
```
